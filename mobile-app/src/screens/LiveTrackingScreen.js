...
// ONE Live Tracking screen for the website AND the Android/iOS app (this
// file used to be LiveTrackingScreen.web.js, with a different, older
// native screen beside it). The few browser-only pieces are guarded by
// IS_WEB: the Leaflet map (phone: components/LiveTrainMap), page
// visibility events (phone: AppState), scrollIntoView (phone: ScrollView
// scrollTo) and the browser Notification API (phone: expo-notifications).
...
export default function LiveTrackingScreen({ navigation }) {
  ...
  // Phone: refs to scroll to the train's current row (web uses scrollIntoView).
  ...
  // Phone: the setting lives in AsyncStorage (async) — load it once.
  useEffect(() => { if (!IS_WEB) loadReadAloudAsync().then(setReadAloud); }, []);
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      if (IS_WEB) {
        ...
        // Phone: position of the row inside the scroll content, then scroll.
  ...
  useEffect(() => {
    ...
    function resumeNow() {
      if (IS_WEB && typeof document !== "undefined" && document.visibilityState === "hidden") return;
    ...
    // Phone: coming back to the app (AppState "active") = page visible again.
    const appStateSub = !IS_WEB ? AppState.addEventListener("change", (st) => { if (st === "active") resumeNow(); }) : null;
    if (IS_WEB && typeof document !== "undefined") document.addEventListener("visibilitychange", resumeNow);
    ...
    return () => {
      ...
      if (IS_WEB && typeof document !== "undefined") document.removeEventListener("visibilitychange", resumeNow);
      if (IS_WEB && typeof window !== "undefined") {
  ...
  return (
    ...
    <View ref={scrollContentRef} collapsable={false}>
      ...
      ) : showMap && activeTrack && payload ? (
        <View style={styles.mapBoxNative}>
          <LiveTrainMap timeline={payload.timeline} lat={payload.lat} lng={payload.lng} trainLabel={`Train ${activeTrack.trainNumber}`} />
        </View>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  mapBoxNative: { marginTop: spacing.sm, marginBottom: spacing.sm, borderRadius: radius.md, overflow: "hidden" },
