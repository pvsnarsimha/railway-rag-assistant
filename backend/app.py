...
def _call_llm_for_synthesis(system_prompt: str, user_prompt: str):
    ...
    final_error = error or gemini_error
...
def _predict_delay_per_reporting_station(
    ...
    """
    FEATURE: per-station predicted delay for EVERY upcoming station —
    reporting halts AND non-reporting/intermediate points alike
    (RailYatri-style for halts, extended on request to cover the small
    in-between stations too, each with its own real distance-anchored
    delay/speed/ETA rather than being silently skipped). Mutates
    `timeline_json` IN PLACE, adding `predicted_delay_minutes` /
    `predicted_delay_confidence` / `predicted_eta` onto every station whose
    `status` is "upcoming" (the currently-occupied station and already-passed
    ones are left alone — they carry RailKit's own REAL recorded/live delay
    already, which must never be overwritten by a model estimate).
    ...
    for stop in timeline_json:
        ...
        if exp_min is not None and eta_min is not None:
            ...
            if diff < -60:  # predicted_eta rolled over past midnight relative to expected
                diff += 1440
...
class TrainSearchRequest(BaseModel):
    ...
    available_only: Optional[bool] = False  # when class+date given: keep/rank by REAL live availability (RailKit), not just which classes the train runs
...
def api_trains_search(req: TrainSearchRequest):
    ...
    # Excludes trains RailKit's own running_days field confirms don't run
    # on the requested weekday (e.g. a Tue/Thu/Sat-only train showing up
    # for a Wednesday search). Never excludes on an unrecognised
    # running_days shape - see trains_between.filter_by_running_date.
    running_day_debug = []
    if req.date:
        before_rd = len(trains)
        trains, running_day_excluded, running_day_debug = trains_between.filter_by_running_date(trains, req.date)
    else:
        running_day_excluded = 0

    time_filter_note = None
    if req.departure_start or req.departure_end or req.arrival_start or req.arrival_end:
        trains, is_exact_time_match = trains_between.filter_by_time_bands(
            trains, req.departure_start, req.departure_end, req.arrival_start, req.arrival_end,
        )
        if trains and not is_exact_time_match:
            time_filter_note = "No trains matched that departure/arrival time — showing all trains on this route instead."
    elif req.time:
    ...
    # Non-GN quotas can make up to 2 live calls per train (primary check +
    # a GN side-check when the primary can't answer, e.g. Tatkal's ARP
    # window) vs GN's 1, so they trip RailKit's rate limit far more easily
    # at the same cap - halve the candidate pool for non-GN quotas so
    # worst-case call volume roughly matches GN's.
    quota_upper = (req.quota or "GN").upper()
    AVAILABILITY_CHECK_CAP = 30 if quota_upper == "GN" else 15
    AVAILABILITY_THROTTLE_SECONDS = 0.3
    MAX_CONSECUTIVE_RATE_LIMITS = 3
    availability_note = None
    availability_cache = {}  # train_number -> (status_text, error)
    rate_limited_skipped = 0
    class_confirmed_absent_count = 0
    gn_check_debug = []
    ...
    def _is_rate_limited(err_text: Optional[str]) -> bool:
        if not err_text:
            return False
        return bool(re.search(r"too many requests|rate limit", err_text, re.IGNORECASE))

    def _class_confirmed_absent(err_text: Optional[str]) -> bool:
        # RailKit's own explicit "this class isn't on this train's route"
        # answer - a real confirmed-negative, not a generic failure like
        # "Too many requests" or an ARP-window restriction. Safe to exclude
        # on, unlike every other error type here which just means "unknown".
        if not err_text:
            return False
        return bool(re.search(r"class does not exist", err_text, re.IGNORECASE))

    if req.travel_class and req.date:
        checked = trains[:AVAILABILITY_CHECK_CAP]
        consecutive_rate_limits = 0
        for i, t in enumerate(checked):
            if consecutive_rate_limits >= MAX_CONSECUTIVE_RATE_LIMITS:
                # RailKit is actively rate-limiting us - stop hammering it.
                # Everything from here on is genuinely "not checked", not
                # "confirmed absent" - keep that distinction honest instead
                # of burning through the rest of the candidates for more
                # of the same error.
                rate_limited_skipped = len(checked) - i
                break
            if i > 0:
                time.sleep(AVAILABILITY_THROTTLE_SECONDS)
            try:
                avail = railway_api.get_seat_availability(
                    t.train_number, source_code, dest_code, req.date, req.travel_class, quota_upper,
                )
                status_text = advanced_features.extract_status_text(avail, target_date=req.date)
                availability_cache[t.train_number] = (status_text, None)
                if status_text is None and availability_raw_keys_sample is None:
                    # A successful call with nothing extractable - capture
                    # the real nested key shape once, so if this is still
                    # missing something, the exact fix is one look away
                    # instead of another guess-and-check round.
                    availability_raw_keys_sample = advanced_features.extract_status_debug_keys(avail)
                consecutive_rate_limits = 0
            except railway_api.RailwayAPIError as e:
                err_text = str(e)
                availability_cache[t.train_number] = (None, err_text)
                if _is_rate_limited(err_text):
                    consecutive_rate_limits += 1
                else:
                    consecutive_rate_limits = 0

        # Trains RailKit has explicitly told us don't run this class on
        # this leg get excluded outright - this is a real confirmed signal,
        # not an unconfirmed one, so it doesn't belong in any fallback
        # bucket below. Everything else (ARP-restricted, rate-limited,
        # generic "unable to process", or never checked at all because it
        # was beyond AVAILABILITY_CHECK_CAP) stays in the pool as unknown.
        class_confirmed_absent_numbers = {
            t.train_number for t in checked
            if _class_confirmed_absent(availability_cache.get(t.train_number, (None, None))[1])
        }

        # GN-quota side-check: a non-GN quota (Tatkal, etc) whose primary
        # check errored NOT because the class is confirmed absent, but
        # because of something quota-specific like Tatkal's ARP booking
        # window not being open yet, tells us nothing about whether the
        # class actually runs on this leg. GN quota has no such booking-
        # window restriction, so for exactly these ambiguous trains we ask
        # GN the one real question that matters - class existence, not live
        # seat status - and only exclude if GN *also* confirms absence.
        gn_check_debug = []
        if quota_upper != "GN":
            ambiguous = [
                t for t in checked
                if t.train_number not in class_confirmed_absent_numbers
                and availability_cache.get(t.train_number, (None, None))[1] is not None
                and not _is_rate_limited(availability_cache[t.train_number][1])
            ]
            for t in ambiguous:
                if consecutive_rate_limits >= MAX_CONSECUTIVE_RATE_LIMITS:
                    break  # RailKit is rate-limiting us - stop, same rule as the primary loop
                time.sleep(AVAILABILITY_THROTTLE_SECONDS)
                try:
                    gn_avail = railway_api.get_seat_availability(
                        t.train_number, source_code, dest_code, req.date, req.travel_class, "GN",
                    )
                    gn_status = advanced_features.extract_status_text(gn_avail, target_date=req.date)
                    gn_check_debug.append({"train_number": t.train_number, "gn_result": gn_status or "responded, no status text"})
                    if gn_status is None and availability_raw_keys_sample is None:
                        availability_raw_keys_sample = advanced_features.extract_status_debug_keys(gn_avail)
                    consecutive_rate_limits = 0
                except railway_api.RailwayAPIError as e:
                    gn_err = str(e)
                    gn_check_debug.append({"train_number": t.train_number, "gn_result": gn_err})
                    if _class_confirmed_absent(gn_err):
                        class_confirmed_absent_numbers.add(t.train_number)
                    if _is_rate_limited(gn_err):
                        consecutive_rate_limits += 1
                    else:
                        consecutive_rate_limits = 0

        class_confirmed_absent_count = len(class_confirmed_absent_numbers)
        if class_confirmed_absent_numbers:
            trains = [t for t in trains if t.train_number not in class_confirmed_absent_numbers]
            checked = [t for t in checked if t.train_number not in class_confirmed_absent_numbers]

        if req.available_only:
            responded = [t for t in checked if availability_cache.get(t.train_number, (None, "x"))[1] is None]
            available_trains = [t for t in responded if _availability_rank(availability_cache[t.train_number][0]) == 0]
            if available_trains:
                trains = available_trains
            elif responded:
                # Nothing fully AVAILABLE, but these trains DID get a real
                # per-route availability response (RAC/WL/etc) from RailKit
                # - i.e. RailKit itself confirms this class actually runs
                # on this leg, just not with open seats right now. That's
                # a much closer signal to IRCTC's own OD-specific listing
                # than the static classes list alone, so prefer this over
                # falling all the way back to the unconfirmed full set.
                trains = sorted(responded, key=lambda t: _availability_rank(availability_cache[t.train_number][0]))
                availability_note = f"No trains show a fully AVAILABLE status for {req.travel_class}/{(req.quota or 'GN').upper()} on {req.date} — showing trains RailKit confirms run this class on this route (RAC/waitlisted), closest to available first."
            else:
                # Every single check errored - couldn't confirm ANYTHING
                # per-route (provider hiccup, or none of these candidates
                # actually run this class on this specific leg). Only now
                # fall back to the coarser classes-list set, and say so.
                availability_note = f"Couldn't confirm live {req.travel_class}/{quota_upper} availability for any train on this route on {req.date}{' (most because Tatkal quota only opens the day before travel)' if quota_upper == 'TQ' else ''} — showing the remaining trains not already ruled out. Try GN quota for advance searches."
                if rate_limited_skipped:
                    availability_note += f" ({rate_limited_skipped} train(s) weren't even attempted — RailKit started rate-limiting this search, so we stopped calling it rather than keep failing.)"
            if class_confirmed_absent_count:
                exclusion_note = f"{class_confirmed_absent_count} train(s) confirmed by RailKit not to run {req.travel_class} on this route were excluded."
                availability_note = f"{exclusion_note} {availability_note}" if availability_note else exclusion_note
        else:
            checked_sorted = sorted(checked, key=lambda t: _availability_rank(availability_cache.get(t.train_number, (None, None))[0]))
            trains = checked_sorted + trains[len(checked):]
    ...
    return {
        ...
        "class_confirmed_absent_count": class_confirmed_absent_count,
        "gn_check_debug": gn_check_debug,
        "availability_raw_keys_sample": availability_raw_keys_sample,
        "running_day_excluded": running_day_excluded,
        "running_day_debug": running_day_debug,
        "availability_note": availability_note,
        "rate_limited_skipped": rate_limited_skipped,
...
def api_coach_layout(travel_class: str, train_number: Optional[str] = None):
...
def api_seat_recommend(req: SeatRecommendRequest):
    return advanced_features.seat_recommend(req.travel_class, req.preferences)
...
def api_route_compare(req: RouteCompareRequest):
    ...
    for train_number in req.train_numbers[:4]:
        ...
        try:
            ...
            entry["availability_status"] = advanced_features.extract_status_text(avail)
        except railway_api.RailwayAPIError as e:
...
def api_fare_heatmap(req: FareHeatmapRequest):
    ...
    for i in range(days):
        ...
        try:
            ...
            cell["status_text"] = advanced_features.extract_status_text(avail)
        except railway_api.RailwayAPIError as e:
...
def api_alerts_check(req: AlertsCheckRequest):
    ...
    for w in req.watches[:8]:
        ...
        try:
            pred = api_delay_predict(DelayPredictRequest(train_number=w.train_number, date=w.date))
            delay = pred.get("predicted_delay_minutes")
